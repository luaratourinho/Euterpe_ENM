...
model <- pa ~bio_2+bio_3+bio_10+bio_15+bio_18
...
## RCP OR GCM 1
raster_files2 <- list.files(paste0("./outputs/", sp.n, "/Fut_env_crop/", BCC), full.names = T, 'tif$|bil$')
head(raster_files2)
...
#### RCP OR GCM 2
raster_files3 <- list.files(paste0("./outputs/", sp.n, "/Fut_env_crop/", IPSL), full.names = T, 'tif$|bil$')
head(raster_files3)
...
#### RCP OR GCM 3
raster_files4 <- list.files(paste0("./outputs/", sp.n, "/Fut_env_crop/", MIROC), full.names = T, 'tif$|bil$')
head(raster_files4)
future_variable3 <- stack(raster_files4)
#future_variable3[[3]] <- future_variable3[[3]]/10 ##divide Temperature values by 10 [https://worldclim.org/data/v1.4/formats.html]
#future_variable3[[4]] <- future_variable3[[4]]/10
#names(future_variable)
...
## pseudoausencia = n*100
sp.data <- read.csv(paste0("./outputs/", sp.n,"/pres_pseudoabs.csv"), header=TRUE, sep=',')

#colocar aqui embaixo  outra planilha depois de rodar pro outro modelo
##pseudoausencia = pres
# To random forest
sp.data2 <- read.csv(paste0("./outputs/", sp.n,"/pres_pseudoabs2.csv"), header=TRUE, sep=',')
...
for(i in 1:k){
  ...
  #mx[[i]] <- maxnet(p = p[[i]], data = data[[i]]) #run maxnet without java. You can use whichever you prefer, maxent or maxnet. You must only erase # from one and add on the other.
...
writeRaster(ens2.future_variable, file = paste(target_dir, '/Fut_bcc.cont_', sp.n, '.asc', sep=""),overwrite=TRUE)
...
writeRaster(ens.future_variable.bin, file = paste(target_dir, '/Fut_bcc.bin_', sp.n, '.asc', sep=""),overwrite=TRUE)
...
writeRaster(future_variable.gm.sd.w, file = paste(target_dir, 'Fut_', BCC, '.gm.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
}
...
writeRaster(future_variable.rf.sd.w, file = paste(target_dir, '/Fut_', BCC, '.rf.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
}
...
writeRaster(future_variable.mx.sd.w, file = paste(target_dir, '/Fut_', BCC, '.mx.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
}
...
writeRaster(future_variable.sv.sd.w, file = paste(target_dir, '/Fut_', BCC, '.sv.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
}
