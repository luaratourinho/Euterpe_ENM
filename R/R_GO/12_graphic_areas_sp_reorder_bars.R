...
p <- ggplot(df2, aes(x=reorder(categ, -value), y=value, fill=variable)) +
...
g <- ggplot(df2, aes(x=reorder(categ, -value), y=value, fill=variable)) +
  geom_bar(stat='identity', position='dodge', width = 0.6) +
  ylim(0, 10000)+
  theme_classic() +
  theme(axis.title.y = element_blank(),
...
ggsave(
  g_arrange,
  file = "./Figures/Species_area_dist_desc_lim.tiff",
  height = 30,
  width = 15,
  units = "cm"
)
